...
\begin{table}[h]
    \noindent\makebox[\textwidth]{\begin{tabular}{| c | c | l | c |}
        ...
        12 & \textit{Swift} & You may play second card this round. & $>>$ \\  \hline
        8 & \textit{Symbiotic} & Double the \textbf{Strength} of your next card with this trait. & $\star$ \\ \hline
        4 & \textit{Poisonous} &\pbox{25cm}{If you control 3 cards with this trait, 
        \\destroy random card played by your opponent.}  & $\dagger$ \\  \hline
        3 & \textit{Empowering} & Your next card's \textit{Attributes} are increased by 1 each. & +\\ \hline
        2 & \textit{Sabotaging} & Your opponent's next card loses random \textit{Attribute} (set it to 0.) & $\downarrow$ \\ \hline
        1 & \textit{Supporting} & Your next card \textbf{Strength} is increased by 1.& $\uparrow$ \\ \hline
    \end{tabular}}
...
node distance=3cm,]
    ...
    \node (step4) [process, below of=step3, label=below right:{\begin{tabular}{c}
         14.5 - 12 $=$ 2.5  \\ 2.5 - 2 $=$ 0.5
    \end{tabular} }] {Step 4 \\ Assign Traits to the card \\ \textit{\textbf{Swift(12), Sabotaging(2)}}};
...
\begin{table}[h]
    \begin{tabular}{| l | l |}
        \toprule
        \multicolumn{2}{|l|}{\textbf{Classname:} 1 } \\
        \midrule
        \multicolumn{2}{|l|}{\textbf{Superclass:} 2 } \\
        \midrule
        \multicolumn{2}{|l|}{\textbf{Subclass(es):} 3 } \\
        \midrule
